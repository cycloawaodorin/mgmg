...
Excel版に比べ，入力のチェックがなされていません．不正な入力に対するエラーメッセージもあまり親切ではありません．
...
`self`が解釈不能な場合，例外が発生します．正当性チェックは厳密ではないため，不正な値のまま処理を進めて，どうにもならなくなってから例外が発生する場合があります．また，製作Lvや完成品の☆制限のチェックを行っていないほか，本ライブラリでは`武器+防具`や`防具+武器`の合成も可能になっています．街の鍛冶・防具製作・道具製作屋に任せた場合をシミュレートする場合は製作Lvを負の値(`-1`など，負であれば何でもよい)にします(製作Lv0相当の性能を計算し，消費エレメント量は委託仕様となります)．
...
### `Mgmg::Equip#inspect`
`Mgmg::Equip#to_s`の出力に加え，0となる9パラ値を省略せず，総消費エレメント量を連結した文字列を出力します．
