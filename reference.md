...
## `Mgmg::IR`
[前述](#stringirleft_associative-true)の`String#ir`または`Enumerable#ir`によって生成される，9パラ値を計算するための，2変数または3変数の関数オブジェクトを保持するクラスです．`Mgmg::IR`では，`Mgmg::Equip`と異なり，重量，EL値，総消費エレメントを取り扱いません．
...
## `Mgmg::Cuisine`
[前述](#mgmgequipreinforce)の`Mgmg::Equip#reinforce`で使用する料理オブジェクトです．料理効果のうち，攻撃，物防，魔防のみが装備を強化できるため，この3パラメータのみを取り扱います．
...
### 引数3つ
攻撃，物防，魔防の強化値を，この順で直接整数で指定します．任意の強化値を指定できますが，強化値は別で調べておく必要があります．代表的な料理の強化値(攻撃，物防，魔防のみ抽出)を以下に示します．

|調理法|主食材|副食材|料理Lv|攻撃|物防|魔防|
|:-|:-|:-|-:|-:|-:|-:|
|焼き|獣肉|カエン酒|0|8|0|0|
|蒸す|ウッチ|氷酒|0|0|11|10|
|蒸す|ゴッチ|氷酒|3|0|15|13|
|蒸す|ガガッチ|氷水酒|6|0|19|15|
|蒸す|ガガッチ|氷河酒|12|0|22|16|
|蒸す|ドランギョ|氷河酒|15|6|24|11|
|蒸す|ドラバーン|氷河酒|24|10|35|19|
|蒸す|アースドラン|氷河酒|27|0|87|0|
|蒸す|ダークドン|氷河酒|27|15|52|38|
|蒸す|ダークドン|煉獄酒|27|21|38|52|
|蒸す|アクアドラン|煉獄酒|27|0|0|87|
